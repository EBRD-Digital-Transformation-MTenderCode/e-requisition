...
import com.procurement.requisition.application.service.set.error.SetLotsStateErrors
...
class SetLotsStateService(
    ...
    fun List<Lot>.setStatus(ids: Set<LotId>, state: LotStateForSettingsRule) = this
        .map { lot ->
            if (lot.id in ids) {
                lot.copy(
                    status = state.status,
