package com.procurement.requisition.json
    ...
    private fun parsing(node: JsonNode, name: String, prefix: String, valueByPath: MutableMap<String, String>) {
        when {
            ...
            node.isArray -> {
                if (node.size() > 0) {
                    ...
                    when (innerNode.nodeType) {
                        ...
                        JsonNodeType.OBJECT -> node.forEachIndexed { index, jsonNode ->
                            parsing(jsonNode, name, "$prefix[$index]", valueByPath)
                        }
                ...
                val value: String = parsePrimitive(node, prefix)
                valueByPath[prefix] = value
            }
        }
    }
